...
//
//jff 4/5/98 return if activated
int P_ActivateInStasisCeiling(Cached<line_t> line)
{
  ceilinglist_t *cl;
  int rtn=0;
...
//
int EV_CeilingCrushStop(Cached<line_t> line)
{
  int rtn=0;
  ...
  for (cl=_g->activeceilings; cl; cl=cl->next)
  {
    ...
    if (ceiling->direction != 0 && ceiling->tag == line->tag)
    {
      ...
      ceiling->direction = 0;
      ceiling->thinker.function.acc1 = NULL;
      rtn=1;
    }
  }
  return rtn;
...
void P_AddActiveCeiling(ceiling_t* ceiling)
{
    ...
    ceilinglist_t *list = (ceilinglist_t *)Z_Malloc(sizeof *list, PU_LEVEL, (void **)&_g->activeceilings);
