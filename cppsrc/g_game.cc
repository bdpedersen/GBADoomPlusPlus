...
void G_DoCompleted (void)
{
   ...
   */
    ...
    int newautomatapmode = _g->automapmode & ~am_active;
    _g->automapmode = (automapmode_e)newautomatapmode;
...
CachedBuffer<uint8_t> G_ReadDemoHeader(CachedBuffer<uint8_t> demo_p, size_t size, boolean failonerror)
{
    ...
    if (_g->demover < 200)     // Autodetect old demos
    {
        ...
        if ((int)(skill=(skill_t)_g->demover) >= 100)         // For demos from versions >= 1.4
        {
            //e6y: check for overrun
