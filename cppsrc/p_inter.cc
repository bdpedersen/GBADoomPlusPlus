...
static void P_KillMobj(mobj_t *source, mobj_t *target)
{
  ...
  if (target->health < -mobjinfo[target->type].spawnhealth && mobjinfo[target->type].xdeathstate)
    P_SetMobjState (target, (statenum_t)mobjinfo[target->type].xdeathstate);
  else
    P_SetMobjState (target, (statenum_t)mobjinfo[target->type].deathstate);
...
void P_DamageMobj(mobj_t *target,mobj_t *inflictor, mobj_t *source, int damage)
{
  ...
  { //killough 11/98: see below
    ...
    P_SetMobjState(target, (statenum_t)mobjinfo[target->type].painstate);
  }
  ...
  P_SetTarget(&target->lastenemy, target->target);
      ...
      if (target->state == &states[mobjinfo[target->type].spawnstate]
        ...
        P_SetMobjState (target, (statenum_t)mobjinfo[target->type].seestate);
    }
