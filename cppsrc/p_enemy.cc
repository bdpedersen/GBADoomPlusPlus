...
void A_Look(mobj_t *actor)
{
   ...
   */
    ...
    P_SetMobjState(actor, (statenum_t)mobjinfo[actor->type].seestate);
}
   ...
   */
    ...
    if (!actor->target || !(actor->target->flags&MF_SHOOTABLE))
    {
        if (!P_LookForTargets(actor,true)) // look for a new target
            P_SetMobjState(actor, (statenum_t)mobjinfo[actor->type].spawnstate); // no new target
        return;
    }
    ...
    if (mobjinfo[actor->type].meleestate && P_CheckMeleeRange(actor))
    {
        ...
        P_SetMobjState(actor, (statenum_t)mobjinfo[actor->type].meleestate);
    ...
    if (mobjinfo[actor->type].missilestate)
    {
        if (!(_g->gameskill < sk_nightmare && actor->movecount))
        {
            ...
            {
                P_SetMobjState(actor, (statenum_t)mobjinfo[actor->type].missilestate);
                actor->flags |= MF_JUSTATTACKED;
                return;
            }
        }
...
void A_CPosRefire(mobj_t *actor)
{
  ...
  if (!actor->target || actor->target->health <= 0
      || !P_CheckSight(actor, actor->target))
stop:  P_SetMobjState(actor, (statenum_t)mobjinfo[actor->type].seestate);
}
        ...
        for (bx=xl ; bx<=xh ; bx++)
        {
            for (by=yl ; by<=yh ; by++)
            {
                ...
                if (!P_BlockThingsIterator(bx,by,PIT_VileCheck))
                {
                    ...
                    P_SetMobjState(_g->corpsehit,(statenum_t)info->raisestate);
...
void A_SpawnFly(mobj_t *mo)
{
  ...
  if (P_LookForTargets(newmobj,true))      /* killough 9/4/98 */
    P_SetMobjState(newmobj, (statenum_t)mobjinfo[newmobj->type].seestate);
