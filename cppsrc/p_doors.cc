  ...
  vldoor_e  type )
{
  ...
  while ((secnum = P_FindSectorFromLineTag(line,secnum)) >= 0)
  {
    ...
    door->thinker.function.acd1 = T_VerticalDoor;
    door->sector = sec;
    door->type = type;
  ...
  mobj_t* thing )
{
  ...
  if (door &&
     ...
     */
    if (door->thinker.function.acd1 == T_VerticalDoor)
    {
       ...
       */
      if (door->thinker.function.acd1 == T_VerticalDoor && door->direction == -1) {
        outval = 1; /* go back up */
      } else if (player) {
      ...
      if (outval) {
        ...
        } else if (door->thinker.function.acl1 == T_PlatRaise) {
