...
void P_SpawnFireFlicker (sector_t*  sector)
{
  ...
  flick->thinker.function.acr1 = T_FireFlicker;
  flick->sector = sector;
...
void P_SpawnLightFlash (sector_t* sector)
{
  ...
  flash->thinker.function.aci1 = T_LightFlash;
  flash->sector = sector;
  ...
  int   inSync )
{
  ...
  flash->thinker.function.aco1 = T_StrobeFlash;
  flash->maxlight = sector->lightlevel;
  flash->minlight = P_FindMinSurroundingLight(sector, sector->lightlevel);
...
//
int EV_StartLightStrobing(Cached<line_t> line)
{
  int   secnum;
  sector_t* sec;
...
// jff 2/12/98 added int return value, fixed return
//
int EV_TurnTagLightsOff(Cached<line_t> line)
{
  int j;
...
// jff 2/12/98 added int return value, fixed return
//
int EV_LightTurnOn(Cached<line_t> line, int bright)
{
  int i;
...
int EV_LightTurnOnPartway(Cached<line_t> line, fixed_t level)
{
  int i;
