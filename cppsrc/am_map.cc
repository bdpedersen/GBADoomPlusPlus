...
static void AM_changeWindowLoc(void)
{
    ...
    {
        int newautomapmode = _g->automapmode & ~am_follow;
        _g->automapmode = (automapmode_e) newautomapmode;
...
static void AM_initVariables(void)
{
    ...
    int newautomapmode = _g->automapmode | am_active;
    _g->automapmode = (automapmode_e) newautomapmode;
...
void AM_Stop (void)
{
    static const event_t st_notify = { (evtype_t)0, ev_keyup, AM_MSGEXITED, 0 };

    _g->automapmode  = (automapmode_e) 0;
    ST_Responder(&st_notify);
    _g->stopped = true;
}
    ...
    else if (ev->type == ev_keydown)
    {
        ...
        else if (ch == key_map)
        {
            if(_g->automapmode & am_overlay)
                ...
                int newautomapmode = _g->automapmode | (am_overlay | am_rotate | am_follow);
                _g->automapmode = (automapmode_e) newautomapmode;
            }
        }
