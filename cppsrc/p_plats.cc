  ...
  int           amount )
{
  ...
  while ((secnum = P_FindSectorFromLineTag(line,secnum)) >= 0)
  {
    ...
    plat->thinker.function.acl1 = T_PlatRaise;
    plat->crush = false;
    plat->tag = line->tag;
...
//
int EV_StopPlat(Cached<line_t> line)
{
  platlist_t *pl;
  for (pl=_g->activeplats; pl; pl=pl->next)  // search the active plats
  {
    ...
    if (plat->status != in_stasis && plat->tag == line->tag)
    {
      ...
      plat->thinker.function.acl1 = NULL;
    }
  }
  return 1;
}
...
void P_AddActivePlat(plat_t* plat)
{
    ...
    platlist_t *list = (platlist_t *)Z_Malloc(sizeof *list, PU_LEVEL, (void**)&_g->activeplats);
    list->plat = plat;
    plat->list = list;
