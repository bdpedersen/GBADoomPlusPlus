...
void *Z_CallocRpt(size_t count, size_t size, int tag, void **user, const char* file, int line) {
    ...
    tagcount_nofree[tag] += count * size;
...
void Z_ReportAll() {
    ...
    for(int i = 0; i < 5; i++) {
        printf("%s%s: Current=%d bytes, Maximum=%d bytes, Without free=%d bytes\n%s", tagcolors[i],tags[i], tagcount[i], maxtagcount[i],tagcount_nofree[i],normaltext);
    }
}
