...
static void R_InitSpriteDefs(const char * const * namelist)
{
  ...
  struct hash_s { int index, next; } *hash;
  int i;
  ...
  _g->sprites = (spritedef_t *)Z_Malloc(_g->numsprites *sizeof(*_g->sprites), PU_STATIC, NULL);
  ...
  hash = (hash_s *)Z_Malloc(sizeof(*hash)*numentries, PU_STATIC, NULL); // allocate hash table
          ...
          if ((_g->sprites[i].numframes = ++_g->maxframe))  // killough 1/31/98
            {
              ...
              _g->sprites[i].spriteframes =
                (spriteframe_t *)Z_Malloc (_g->maxframe * sizeof(spriteframe_t), PU_STATIC, NULL);
